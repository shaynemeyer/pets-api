...
class PetAPI(MethodView):
    ...
    def get(self, pet_id):
        if pet_id:
            ...
            pets = pets.paginate(page=page, per_page=self.PET_PER_PAGE)
            response = {
                "result": "ok",
                "links": [
                    {
    ...
    def post(self):
        ...
        try:
            received_date = datetime.datetime.strptime(pet_json.get('received_date'), "%Y-%m-%dT%H:%M:%SZ")
        except:
